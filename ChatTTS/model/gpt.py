        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                ...
                if hasattr(past_key_values, "get_max_length"):
                    max_cache_len_val = past_key_values.get_max_length()
                elif hasattr(past_key_values, "get_max_cache_shape"):
                    max_cache_len_val = past_key_values.get_max_cache_shape()
                else:
                    max_cache_len_val = None
                
                if max_cache_len_val is not None and max_cache_len_val >= 0:
                    max_cache_length = torch.tensor(max_cache_len_val, device=input_ids.device)
                else:
